<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <meta content="width=device-width, initial-scale=1.0" name="viewport">

    <title>Foreign Language College Mombasa - Feestructure</title>
    <meta content="" name="description">
    <meta content="" name="keywords">

    <?php include 'css.php' ?>

</head>

<body>

    <!-- ======= Header ======= -->
    <?php include 'header.php' ?>
    <main id="main">
        <!-- ======= Breadcrumbs ======= -->
        <div class="breadcrumbs" data-aos="fade-in">
            <div class="container">
                <h2>Portuguese Course Fee</h2>
                <p>Breakdown of our Portuguese Fee Structure. </p>
            </div>
        </div><!-- End Breadcrumbs -->

        <!-- ======= Pricing Section ======= -->
        <section id="pricing" class="pricing">
            <div class="container" data-aos="fade-up">
                <a href="/Scholarship"><button class="btn btn-success btn-lg">Apply Scholarship</button></a>
                <br /><br />
                <p>Scholarships are open from 1st - 15th January for the first 40 Students who apply. You get to qualify for 30% off!!!.</p>
                <div class="row">
                    <!--<div class="col-lg-3 col-md-6">-->
                    <!--  <div class="box">-->
                    <!--    <h3>Regular Plan</h3>-->
                    <!--    <h4><sup>Ksh</sup>8,000<span> / month</span></h4>-->
                    <!--    <ul>-->
                    <!--      <li>Duration: 2 housr per session for 3 months</li>-->
                    <!--      <li>Physical/Online/Blended Classes</li>-->
                    <!--      <li> Registration fees: Kshs. 1,000</li>-->
                    <!--      <li>Tuition fees: Kshs. 36,000</li>-->
                    <!--      <li>Certificate and Assessment fees: Kshs. 4000</li>                -->
                    <!--      <li style="color:red;font-size:20px;"><b>TOTAL AMOUNT: Kshs. 41,000</b></li>-->
                    <!--    </ul>-->
                    <!--    <div class="btn-wrap">-->
                    <!--      <a href="#" class="btn-buy">Enroll Now</a>-->
                    <!--    </div>-->
                    <!--  </div>-->
                    <!--</div>-->

                    <div class="col-lg-4 col-md-6 mt-4 mt-md-0">
                        <div class="box featured">
                            <h3>Intensive Plan</h3>
                            <h4><sup>Ksh</sup>9,125<span> / month</span></h4>
                            <ul>
                                <li>Duration: 2 hours per day for 4 months</li>
                                <li>Physical/Online/Blended Classes</li>
                                <li>Books: Kshs 4,500</li>
                                <li>Admission fees: Kshs 3,000</li>
                                <li>Tuition Fees: Kshs. 36,500</li>
                                <li style="color:red;font-size:20px;"><b>TOTAL AMOUNT: Kshs. 44,000</b></li>
                            </ul>
                            <div class="btn-wrap">
                                <a href="/Admission" class="btn-buy">Enroll Now</a>
                            </div>
                        </div>
                    </div>

                    <div class="col-lg-4 col-md-6 mt-4 mt-md-0">
                        <div class="box featured">
                            <h3>Super Intensive Plan</h3>
                            <h4><sup>Ksh</sup>18,250<span> / month</span></h4>
                            <ul>
                                <li>Duration: 3 hours per day for 2 months</li>
                                <li>Physical/Online/Blended Classes</li>
                                <li>Books: Kshs 4,500</li>
                                <li>Admission fees: Kshs 3,000</li>
                                <li>Tuition Fees: Kshs. 36,500</li>
                                <li style="color:red;font-size:20px;"><b>TOTAL AMOUNT: Kshs. 44,000</b></li>
                            </ul>
                            <div class="btn-wrap">
                                <a href="/Admission" class="btn-buy">Enroll Now</a>
                            </div>
                        </div>
                    </div>

                    <div class="col-lg-4 col-md-6 mt-4 mt-md-0">
                        <div class="box featured">
                            <h3>ACCELERATED Plan</h3>
                            <h4><sup>Ksh</sup>36,500<span> / month</span></h4>
                            <ul>
                                <li>Duration: 6 hours per day for 1 month</li>
                                <li>Physical/Online/Blended Classes</li>
                                <li>Books: Kshs 4,500</li>
                                <li>Admission fees: Kshs 3,000</li>
                                <li>Tuition Fees: Kshs. 36,500</li>
                                <li style="color:red;font-size:20px;"><b>TOTAL AMOUNT: Kshs. 44,000</b></li>
                            </ul>
                            <div class="btn-wrap">
                                <a href="/Admission" class="btn-buy">Enroll Now</a>
                            </div>
                        </div>
                    </div>
                    <img src="/assets/img/Portuguese-Fee-Structure-Updated.jpg" class="img-fluid" />
                </div>

            </div>
        </section><!-- End Pricing Section -->

    </main><!-- End #main -->

    <!-- ======= Footer ======= -->
    <?php include 'footer.php' ?>

</body>

</html>